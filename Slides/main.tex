...
\end{lstlisting}
  ...
  \end{columns}
  \vspace{0.2cm}
  \takeaway{A common interface makes honest comparisons cheap: a baseline is one line away.}
\end{frame}
...
\end{lstlisting}
      {\small Mean absolute error, pooled over 7 folds: mean score \textbf{0.184}, RDST \textbf{0.084}, FCN \textbf{0.095}.}
    \end{column}
    \begin{column}{0.48\textwidth}
  ...
  \end{columns}
  \vspace{0.1cm}
  \takeaway{Always report a trivial baseline, and a non-deep one: here RDST matches the network for a fraction of the cost.}
\end{frame}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.29\textwidth}
      \begin{itemize}
        ...
        \item Model-agnostic: works with RDST too.
...
\begin{frame}{The same code on clinical data: KIMORE}
  ...
  \end{columns}
  \vspace{0.15cm}
  \takeaway{Small clinical datasets are hard, in the published benchmark too. This is where baselines earn their place.}
\end{frame}
  ...
  \begin{columns}[t]
    \begin{column}{0.60\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Evaluate over all folds}, and report trivial and non-deep baselines.
