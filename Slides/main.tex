...
\begin{frame}[plain]
  ...
  \vfill
  \begin{center}
    \raisebox{-0.5\height}{\includegraphics[height=0.72cm]{images/logo-irimas-uha.png}}\hspace{1.0cm}%
    \raisebox{-0.5\height}{\includegraphics[height=0.50cm]{images/logo-novahe.png}}\hspace{1.0cm}%
    \raisebox{-0.5\height}{\includegraphics[height=1.10cm]{images/logo-p16.png}}\hspace{1.0cm}%
    \raisebox{-0.5\height}{\includegraphics[height=0.48cm]{images/logo-inria.png}}\hspace{1.0cm}%
    \raisebox{-0.5\height}{\includegraphics[height=0.54cm]{images/logo-anr.png}}%
  \end{center}
  \vspace{0.15cm}
\end{frame}
...
\begin{frame}{Encoder: convolutions followed by attention}
  ...
  \vspace{0.35cm}
  \begin{itemize}
    \item A dot-product self-attention over time re-weights the last feature maps. The original paper instead splits them in half, one half weighting the other.
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Encoder}: weights tied to positions, it fails once the movement is elsewhere. The \textbf{MLP} never learns this problem at our budget.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.15cm}
...
\end{lstlisting}
  \vspace{0.1cm}
  \begin{itemize}
    \item Running example: the \textbf{hurdle step} of UI-PRMD. Seven healthy subjects, correct and incorrect executions.
    \item Same series, two targets: \texttt{UIPRMD\_clf\_bn\_HS} (label) and \texttt{UIPRMD\_reg\_HS} (quality score).
    \item Before training: Each axis is mapped to $[0, 1]$ by its minimum and maximum, taken from the \textbf{training} split and reused on the test split.
  \end{itemize}
\end{frame}
...
fcn.predict_proba(X_test[:2])      # array([[0.69, 0.31], [0.97, 0.03]])
\end{lstlisting}
  \vspace{0.1cm}
  \begin{itemize}
    ...
    \item Fold 0: 18 test recordings, 16 correct. \textbf{One fold, 18 recordings: enough to trust?}
  \end{itemize}
\end{frame}
...
\end{lstlisting}
      \begin{itemize}
        \item The 7 test sets are disjoint and cover all 110 recordings: \textbf{pool} the predictions. We run the first 3 here, 54 recordings; set \texttt{N\_FOLDS = 7} for the full protocol.
        \item Fold 0: accuracy 0.89. Three folds pooled: \textbf{0.93}.
      \end{itemize}
    \end{column}
    \begin{column}{0.34\textwidth}
      \centering
...
\begin{frame}{Two practical lessons for small training budgets}
  \begin{columns}[t]
    \begin{column}{0.48\textwidth}
      \textbf{1. Use the archive's normalisation}
      \begin{itemize}
        \item Rehab-Pile prescribes one: each axis to $[0, 1]$, from the \textbf{training} split's minimum and maximum.
        \item Take it as given. Preprocessing of your own moves every number and breaks comparison with the published results.
        \item It rescales, it does not centre. At 100 epochs the simplest models pay for that: MLP 0.56 on fold 0.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{2. Mind the batch size}
      \begin{itemize}
        \item Defaults follow the papers: mostly 1500 to 2000 epochs, batches of 16 to 64 (the Encoder excepted, at 100 and 12).
        \item 92 recordings, 100 epochs, batches of 64: about 200 gradient steps.
        \item Too few for \textbf{batch normalisation} statistics to settle: at batches of 64, Inception collapses to a single class on most folds and ResNet on some (0.56 and 0.70). Batches of 16 recover both (0.89 and 0.85).
      \end{itemize}
    \end{column}
  \end{columns}
  \vfill
...
evaluate(lambda: FCNRegressor(n_epochs=100),
         "UIPRMD_reg_HS")
\end{lstlisting}
      {\small Mean absolute error, pooled over 3 folds: mean score \textbf{0.198}, FCN \textbf{0.104} (correlation 0.82).}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{The published benchmark answers ``which model is best?''}
  \begin{columns}[c]
    ...
    \begin{column}{0.46\textwidth}
      \begin{itemize}
        \item 60 problems (39 classification, 21 regression), 1500 epochs, five runs, ensembles: a question for a benchmark, not for a laptop.
...
\begin{frame}{When does the model decide? Class activation maps}
  ...
  \begin{tikzpicture}[
      blk/.style={draw=aeonblue, fill=aeonblue!8, rounded corners=2pt, align=center,
                  minimum height=0.9cm, minimum width=1.5cm, font=\scriptsize},
      obox/.style={blk, fill=aeonorange!18, draw=aeonorange},
      arr/.style={-{Latex[length=1.8mm]}, muted, thick},
      arro/.style={-{Latex[length=1.8mm]}, aeonorange, thick}]
    \node[blk, fill=white, minimum width=1.0cm] (in) {series};
    \node[blk, right=0.3cm of in, minimum width=1.5cm] (conv) {3 conv blocks};
    \node[draw=aeonblue, fill=white, rounded corners=2pt,
          minimum width=2.0cm, minimum height=1.2cm, right=0.45cm of conv] (fm) {};
    \node[font=\scriptsize, above=1pt of fm, xshift=-0.35cm] {$K$ maps $A_k(t)$, one per filter};
    \begin{scope}[shift={(fm.center)}]
      \draw[aeonblue, semithick] (-0.85,0.36) .. controls (-0.5,0.50) and (-0.3,0.24) .. (0.0,0.42)
                                 .. controls (0.3,0.56) and (0.5,0.28) .. (0.85,0.38);
      \draw[aeonblue!65, semithick] (-0.85,0.02) .. controls (-0.55,-0.12) and (-0.2,0.16) .. (0.1,0.0)
                                 .. controls (0.4,-0.14) and (0.6,0.10) .. (0.85,-0.02);
      \draw[aeonblue!40, semithick] (-0.85,-0.36) .. controls (-0.45,-0.22) and (-0.15,-0.48) .. (0.2,-0.30)
                                 .. controls (0.5,-0.18) and (0.65,-0.44) .. (0.85,-0.34);
    \end{scope}
    \node[blk, right=1.1cm of fm, yshift=0.8cm, minimum width=2.3cm] (gap)
         {GAP over $t$\\[1pt]$\bar{A}_k=\frac{1}{m}\sum_t A_k(t)$};
    \node[blk, right=0.3cm of gap, minimum width=2.3cm] (dense)
         {dense $w^c$\\[1pt]$s_c=\sum_k w_k^c\,\bar{A}_k$\\[1pt]{\tiny\color{muted} then softmax $\rightarrow P(c)$}};
    \node[obox, right=1.1cm of fm, yshift=-0.9cm, minimum width=5.0cm] (cam)
         {\textbf{CAM}: the same weights, without the average\\[1pt]$\mathrm{CAM}_c(t)=\sum_k w_k^c\,A_k(t)$};
    \draw[arr] (in) -- (conv);
    \draw[arr] (conv) -- (fm);
    \draw[arr] (fm.east) -- ++(0.3,0) |- (gap.west);
    \draw[arr] (gap) -- (dense);
    \draw[arro] (fm.east) -- ++(0.3,0) |- (cam.west);
  \end{tikzpicture}
  \vspace{-0.05cm}
  \begin{columns}[c]
    \begin{column}{0.37\textwidth}
      \begin{itemize}
        \item Average it back over time and $s_c$ returns: the CAM splits the score over the movement.
        \item GAP feeding a single linear layer: FCN, ResNet, Inception, LITE. A few lines on \texttt{fcn.model\_}.
    ...
    \end{column}
  \end{columns}
  \source{Zhou et al. Learning deep features for discriminative localization. CVPR 2016 (constant bias left out, as in the paper)}
\end{frame}
...
\begin{frame}{Where on the body? Occlusion, for any estimator}
  \begin{columns}[c]
    \begin{column}{0.50\textwidth}
      ...
      {\scriptsize\color{muted} Mean CAM for \emph{incorrect}, grouped by true class.}
    \end{column}
    ...
    \begin{column}{0.29\textwidth}
      \begin{itemize}
        ...
        \item The decision rests on the \textbf{lifted leg}: ankle and foot dominate, the knee far behind.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.05cm}
...
\begin{frame}{The same code on clinical data: KIMORE}
  \begin{columns}[c]
    ...
    \begin{column}{0.42\textwidth}
      \begin{itemize}
        \item 78 people, 34 with motor dysfunctions; expert score out of 100; 18 joints, 557 time points.
        \item Loading and plotting change only by the problem name and the kinematic tree; the models add two one-liners, a downsampler and a rescaler for the 0--100 score.
      \end{itemize}
      \centering\small
      \begin{tabular}{@{}lr@{}}
        \toprule
